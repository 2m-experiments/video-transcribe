...
| [`bundle/`](bundle/README.md) | Transcripts packaged for a ChatGPT / Claude Project so you can ask questions by voice from a phone. Regenerated by `bundle.py`. |
...
4. Publish dates        video_meta.py                       (channel/video_meta.json)
5. Build index          index.py build --group <name>
6. Build bundle         bundle.py                           (bundle/ for a ChatGPT/Claude Project)
7. Query index          index.py query --group <name> "your question"
```
...
## 4. Bundle for a phone / voice workflow

`bundle.py` packages everything into ~15 large Markdown files under `bundle/`: a catalog of
all episodes newest-first (publish date, speakers, summary, topics, URL) plus chronological
chunk files where every episode section carries its metadata right above the full
speaker-labelled transcript. Upload those files once as knowledge in a ChatGPT or Claude
Project and ask by voice; `bundle/README.md` has the steps and suggested project
instructions. Publish dates come from `video_meta.py` (run it first), and after a new batch
`bundle.py` tells you which files changed so only those need re-uploading.

```bash
python video_meta.py     # fetch publish dates for new videos (incremental)
python bundle.py         # rebuild bundle/, print changed files
```
...
# 4. Publish dates, summary index, phone bundle
python video_meta.py
