...
def main() -> int:
    ...
    ap.add_argument("--budget", type=int, default=FULL_TEXT_BUDGET,
                    help="max chars of full transcript text (newest first); older episodes become summary-only. 0 = no limit")
    args = ap.parse_args()
    ...
    eps = build_episodes()
    # Newest first: full text until the budget is spent, then summary-only.
    used, over = 0, False
    for e in sorted(eps, key=lambda e: (e["date"] or e["transcribed"] or "", e["title"]), reverse=True):
        over = over or (bool(args.budget) and used + len(e["text"]) > args.budget)
        if over:  # strict cutoff: everything older than the first overflow is summary-only
            e["digest"] = True
            e["text"] = ""
        else:
            used += len(e["text"])
    ...
    for e in eps:
        by_label["zz_digest" if e.get("digest") else LABELS.get(e["group"], e["group"])].append(e)
    ...
    for e in sorted(eps, key=lambda e: (e["date"] or e["transcribed"] or "", e["title"]), reverse=True):
        ...
        bits.append(e["url"])
        bits.append(f"fil: {e['bundle_file']}" + (" (kun resumé)" if e.get("digest") else ""))
...
{len(eps)} episodes in {len(files)} files: {sum(1 for e in eps if not e.get("digest"))} with full transcripts
(newest first, within a {args.budget:,}-char budget so a Claude Project accepts the upload) and
{sum(1 for e in eps if e.get("digest"))} older ones as summary-only digest entries. `python bundle.py --budget 0`
